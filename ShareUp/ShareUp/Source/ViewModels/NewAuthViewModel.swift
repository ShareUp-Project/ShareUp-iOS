...
class NewAuthViewModel: ViewModelType {
    ...
    func transform(_ input: Input) -> Output {
        ...
        let resetInfo = Driver.combineLatest(input.phoneNum, input.againPassword)
        let isEquals = checkInfo.map { !$0.1.isEmpty && !$0.0.isEmpty && $0.0 == $0.1 ? true : false }
        
        input.resetTap.asObservable().withLatestFrom(resetInfo).subscribe(onNext: {[weak self] phone, password in
        ...
        return Output(result: result.asSignal(onErrorJustReturn: ""), check: check.asSignal(onErrorJustReturn: ""), isEquals: isEquals.asDriver())
    }
}
