...
final class PostViewModel: ViewModelType {
    ...
    struct Output {
        ...
        let stopIndicator: Signal<String>
    }
    ...
    func transform(_ input: Input) -> Output {
        ...
        let stop = PublishSubject<String>()
        ...
        input.postTap.asObservable()
            ...
            .subscribe(onNext: { data, response in
                ...
                case .ok:
                    ...
                    stop.onCompleted()
                default:
                    stop.onNext("글 쓰기를 다시 시도해보세요.")
                }
            }).disposed(by: disposeBag)
        
        return Output(result: result.asDriver(onErrorJustReturn: nil), isEnable: isEnable.asDriver(onErrorJustReturn: false), stopIndicator: stop.asSignal(onErrorJustReturn: ""))
    }
}
